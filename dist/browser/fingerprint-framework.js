...
(function attachFingerprintFramework(root) {
  'use strict';
  const VERSION = '0.1.0';
  const SCHEMA_VERSION = 'ff-v1';
  const DEFAULT_COLLECTOR_TIMEOUT_MS = 700;
  
  const SENSITIVITY_RANK = Object.freeze({ low: 1, medium: 2, high: 3 });
  ...
  async function hashValue(value, runtime = {}) {
    ...
    const cryptoRef = runtime.crypto || getGlobal().crypto || null;
  ...
  function withTimeout(promise, timeoutMs, collectorId) {
    const globalRef = getGlobal();
    const setTimer = typeof setTimeout === 'function' ? setTimeout : globalRef.setTimeout;
    const clearTimer = typeof clearTimeout === 'function' ? clearTimeout : globalRef.clearTimeout;
  ...
  function normalizeBrands(brands) {
    ...
    return brands
      .map((brand) => ({ brand: brand.brand || null, version: brand.version || null }))
